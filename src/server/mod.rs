...
//! 持久化经 db facade（sqlx 连接池）：默认 SQLite 零配置单机，
//! 生产可经 `TRISKELION_DATABASE_URL` 切换 PostgreSQL / MySQL。
...
pub fn run() -> Result<()> {
    ...
    // 数据库连接串：缺省为 <data_dir>/hub.db（SQLite，零配置）。
    let db_url = std::env::var("TRISKELION_DATABASE_URL").ok();
    ...
    rt.block_on(async move {
        let db = Db::connect(db_url.as_deref(), &data_dir).await?;
        db::schema::init(&db).await?;
